(function () {
  ...
  function getProjects() {
    return Array.isArray(window.INTERIOR_PROJECTS) ? window.INTERIOR_PROJECTS : [];
  }

  function getCachedProject(slug) {
    try {
      const raw = sessionStorage.getItem('sayhomesInteriorProjectCurrent');
      if (!raw) return null;
      const parsed = JSON.parse(raw);
      if (parsed && parsed.slug === slug && parsed.project) return parsed.project;
    } catch (e) {}
    return null;
  }
  ...
  function renderProjectDetail(container, projects, slug) {
    ...
    const cached = getCachedProject(normalizedSlug);
    const selectedProject = cached || getProjectBySlug(projectList, normalizedSlug);
    const currentIndex = projectList.findIndex(
      (project) => slugify(`${project.owner}-${project.title}`) === normalizedSlug
    );
    ...
    container.innerHTML = `
      <section class="hero-card">
        <div class="hero-content">
          <p class="hero-kicker">Interior Project</p>
          <h1 class="hero-title">${selectedProject.owner || 'Interior Project'}</h1>
          <p class="hero-subtitle">${selectedProject.title || 'Residential Interior'}</p>
          <a href="https://wa.me/919686326936?text=${encodeURIComponent('Hi SayHomes, I am interested in a similar interior project like ' + (selectedProject.owner || '') + ' — ' + (selectedProject.title || '') + '.')}" class="hero-wa-cta" target="_blank" rel="noopener noreferrer">
    ...
    function goToProject(index) {
      ...
      try {
        sessionStorage.setItem(
          'sayhomesInteriorProjectCurrent',
          JSON.stringify({
            slug: slugify(`${targetProject.owner}-${targetProject.title}`),
            project: targetProject,
          })
        );
      } catch (e) {}

      window.location.href = `interior-project-detail.html?project=${slugify(`${targetProject.owner}-${targetProject.title}`)}`;
    }
  ...
  window.SayHomesInteriorProjectDetail = {
    slugify,
    renderProjectDetail,
  };
})();
